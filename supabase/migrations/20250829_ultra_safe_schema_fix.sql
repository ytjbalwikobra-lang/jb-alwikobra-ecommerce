-- ULTRA-SAFE schema fix - checks actual production schema first
...
DO $$
DECLARE
    has_tiers_table BOOLEAN := FALSE;
    has_game_titles_table BOOLEAN := FALSE;
    has_category_column BOOLEAN := FALSE;
    has_tier_id_column BOOLEAN := FALSE;
    has_game_title_id_column BOOLEAN := FALSE;
    has_account_level_column BOOLEAN := FALSE;
    has_account_details_column BOOLEAN := FALSE;
    has_images_column BOOLEAN := FALSE;
BEGIN
    -- Check if tables exist
    SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tiers') INTO has_tiers_table;
    SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'game_titles') INTO has_game_titles_table;
    
    -- Check if specific columns exist
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'game_titles' AND column_name = 'category') INTO has_category_column;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'tier_id') INTO has_tier_id_column;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'game_title_id') INTO has_game_title_id_column;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'account_level') INTO has_account_level_column;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'account_details') INTO has_account_details_column;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'images') INTO has_images_column;
    
    -- Create tiers table if it doesn't exist
    IF NOT has_tiers_table THEN
        CREATE TABLE tiers (
        ...
        RAISE NOTICE 'Created tiers table';
    END IF;
    
    -- Create game_titles table if it doesn't exist (without category for now)
    IF NOT has_game_titles_table THEN
        ...
        RAISE NOTICE 'Created game_titles table without category column';
    END IF;
    
    -- Add category column to game_titles if it doesn't exist
    IF has_game_titles_table AND NOT has_category_column THEN
        ALTER TABLE game_titles ADD COLUMN category VARCHAR(50);
        RAISE NOTICE 'Added category column to game_titles';
    END IF;
    
    -- Add missing columns to products table
    IF NOT has_tier_id_column THEN
        ALTER TABLE products ADD COLUMN tier_id UUID;
        RAISE NOTICE 'Added tier_id column to products';
    END IF;
    
    IF NOT has_game_title_id_column THEN
        ALTER TABLE products ADD COLUMN game_title_id UUID;
        RAISE NOTICE 'Added game_title_id column to products';
    END IF;
    
    IF NOT has_account_level_column THEN
        ALTER TABLE products ADD COLUMN account_level VARCHAR(100);
        RAISE NOTICE 'Added account_level column to products';
    END IF;
    
    IF NOT has_account_details_column THEN
        ALTER TABLE products ADD COLUMN account_details TEXT;
        RAISE NOTICE 'Added account_details column to products';
    END IF;
    
    IF NOT has_images_column THEN
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_products_tier') THEN
        ...
        RAISE NOTICE 'Added tier foreign key constraint';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_products_game_title') THEN
        ...
        RAISE NOTICE 'Added game_title foreign key constraint';
    END IF;
END $$;

-- Step 3: Insert tier data safely
...
-- Step 4: Insert game titles data (check if category column exists first)
DO $$
DECLARE
...
BEGIN
    ...
    IF has_category THEN
        ...
        ON CONFLICT (slug) DO UPDATE SET
            ...
            updated_at = NOW();
    ELSE
        -- Insert without category column
    ...
    RAISE NOTICE 'Inserted game titles data (with category: %)', has_category;
END $$;

-- Step 5: Update existing products to have proper foreign key relationships
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'game_titles' AND column_name = 'category') THEN
...
-- Step 8: Create RLS policies safely
DO $$
BEGIN
...
-- Step 9: Create view (adapts to whether category column exists)
DO $$
DECLARE
    has_category BOOLEAN;
    view_sql TEXT;
BEGIN
    ...
    ELSE
        ...
        SELECT 
            ...
            gt.slug as game_title_slug,
            gt.description as game_title_description,
            NULL::VARCHAR(50) as game_category,
            gt.icon as game_icon,
            gt.color as game_color,
            gt.logo_url as game_logo_url,
            gt.is_popular as game_is_popular
    ...
    EXECUTE view_sql;
    RAISE NOTICE 'Created view with category support: %', has_category;
END $$;

-- Grant access to the view
GRANT SELECT ON products_with_details TO anon, authenticated;

-- Step 10: Insert sample products only if table is empty
...
SELECT 
    ...
    (SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'game_titles' AND column_name = 'category')) as has_category_column;

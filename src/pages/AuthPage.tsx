...
import { Lock, Mail, User, Eye, EyeOff, ArrowLeft, Sparkles } from 'lucide-react';
...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'signup') {
        const { error } = await supabase.auth.signUp({ email, password, options: { data: { name: email.split('@')[0], role: 'user' } } });

...
const PaymentStatusPage: React.FC = () => {
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Tidak ada detail order</h2>
          <p className="text-gray-600 mb-4">Jika pembayaran berhasil, Anda akan menerima email konfirmasi. {authed ? 'Cek juga riwayat order Anda.' : 'Silakan login untuk melihat riwayat order.'}</p>
          <div className="flex gap-3 justify-center">
            {authed && <Link to="/orders" className="bg-primary-600 text-white px-4 py-2 rounded-lg">Lihat Riwayat Order</Link>}
            <Link to="/" className="border px-4 py-2 rounded-lg">Beranda</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className={`p-6 rounded-xl border ${style.bg}`}>
          <h1 className={`text-2xl font-bold mb-2 ${style.color}`}>{style.title}</h1>
          <p className="text-gray-700 mb-4">Order ID: <span className="font-mono">{order.id}</span></p>
          <p className="text-gray-700 mb-6">Jumlah: <strong>Rp {Number(order.amount).toLocaleString('id-ID')}</strong></p>
          <div className="text-sm text-gray-700 space-y-1 mb-6">
            ...
            {order.xendit_invoice_url && <p><a href={order.xendit_invoice_url} target="_blank" rel="noreferrer" className="text-primary-600">Buka invoice Xendit</a></p>}
          ...
          <div className="flex gap-3">
            <Link to="/products" className="bg-primary-600 text-white px-4 py-2 rounded-lg">Lanjut belanja</Link>
            <Link to="/" className="border px-4 py-2 rounded-lg">Beranda</Link>
          </div>
        </div>
      </div>

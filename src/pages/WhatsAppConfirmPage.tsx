...
import { CheckCircle, AlertCircle, MessageCircle, ArrowRight } from 'lucide-react';
...
const WhatsAppConfirmPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!token) {
      setStatus('error');
      setMessage('Token konfirmasi tidak ditemukan dalam URL');
      return;
    }
  ...
  const confirmWhatsAppAuth = async () => {
    try {
      ...
      if (result.success) {
        ...
        setStatus('error');
        setMessage(result.error || 'Konfirmasi gagal');
      }
    } catch (error) {
      setStatus('error');
  ...
  const handleContinue = () => {
    if (userExists) {
      ...
      navigate('/auth?mode=signin&message=Akun berhasil dibuat! Silakan login dengan password yang dikirim via WhatsApp.');
    }
  };
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">
            Memverifikasi konfirmasi akun Anda...
          </p>
        </div>
              ...
              {!userExists && (
                <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-4">
                  <div className="flex items-center space-x-2 text-green-400 mb-2">
                    ...
                    <span className="font-medium">Password Dikirim via WhatsApp</span>
                  </div>
                  <p className="text-sm text-green-300">
                    Silakan cek WhatsApp Anda untuk mendapatkan password sementara.
                  </p>
                </div>
              )}
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="text-blue-400 font-medium mb-1">
                Tentang Konfirmasi WhatsApp
              </h3>
              <p className="text-blue-300 text-sm">
                Kami menggunakan WhatsApp untuk konfirmasi yang lebih cepat dan mudah. 

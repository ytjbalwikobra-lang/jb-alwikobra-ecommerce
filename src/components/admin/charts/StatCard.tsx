...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon: Icon, change, subtitle, loading = false, className = '' }) => {
  if (loading) {
    return (
  ...
  const changeColor = change ? (change.type === 'increase' ? 'text-green-400' : 'text-red-400') : '';
      ...
      <div className="space-y-2">
        ...
        {change && (
          <div className="flex items-center space-x-1">
            <span className={`text-sm font-medium ${changeColor}`}>{change.type === 'increase' ? '+' : '-'}{Math.abs(change.value)}%</span>
